...
import type { CreateModuleProps, IModuleClientAPI } from "./types";
...
const framework = (function () {
  ...
  return {
    ...
    createModule<S = any, M = any, G = any, A = any, L = any>(
      moduleProps: CreateModuleProps<S, M, G, A, L>
    ) {
      if (!hasModule(moduleProps.name)) {
        const moduleInstance = createModuleInstance(moduleProps);
        createModule(moduleProps.name, moduleInstance);
      }
       ...
       *
       * @property {Function} createEntity - Creates a new entity instance within the module.
       ...
       *
       * @property {Function} getEntityById - Retrieves an entity by its unique identifier.
       ...
       *
       * @property {Function} removeEntity - Removes an entity from the module by its id.
      ...
      return getModuleByName(moduleProps.name) as IModuleClientAPI<
        S,
        M,
        G,
        A,
